...
display_summary <- function(summary_results) {
  i = 1
  for (x in names(summary_results)) {
    cat(paste0("Parametr phi: ", ar_values[i], "\n"))
    print(summary_results[[x]])
    cat("\n")
    i = i + 1
  }
}
...
summary_results <- analyze_all(results, ar_values)

# Wyświetlanie podsumowania
display_summary(summary_results)

# Analiza dla konkretnej długości danych (N=50)
N_focus <- "N_50"
...
# Analiza dla konkretnej długości danych (N=100)
N_focus <- "N_100"
estimates_all <- combine_estimates_for_N(results, N_focus)
...
# Tworzenie histogramów N_1000
plot_histograms(estimates_df_long)
```

```{r}
generate_ar3_data <- function(N, params = c(0.1, 0.5, 0.9)) {
...
```{r}
analyze_single_simulation <- function(N, params = c(0.1, 0.5, 0.9)) {
...
```{r}
run_simulation_ar3_to_ar1 <- function(n_values, n_reps, params = c(0.1, 0.5, 0.9)) {
  results <- vector("list", n_reps)
...
```{r}
# Parametry
true_param_ar1 <- 0.5  # Prawdziwa wartość parametru AR(1)
...
```{r}
# Parametry
true_param_ar1 <- 0.9  # Prawdziwa wartość parametru AR(1)
